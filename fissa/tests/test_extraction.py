...
from ..extraction import DataHandler,  DataHandlerPillow
from .. import roitools

class TestImage2ArrayTifffile(BaseTestCase):
datahandler = DataHandler()
datahandlerpillow = DataHandlerPillow()
...
class TestImage2ArrayPillow(BaseTestCase):
    ...
    def test_actual_tiff(self):
        # load from tif
        actual = datahandlerpillow.image2array('test.tif')
...
class TestRois2MasksPillow(BaseTestCase):
    ...
    def test_imagej_zip(self):
        # load zip of rois
        ROI_loc = os.path.join(self.test_directory, 'resources', 'RoiSet.zip')
        actual = datahandlerpillow.rois2masks(ROI_loc, self.data)
    ...
    def test_arrays(self):
        # load from array
        actual = datahandlerpillow.rois2masks(self.polys, self.data)
        # assert equality
        self.assert_equal(actual, self.expected)

    def test_transposed_polys(self):
        # load from array
        actual = datahandlerpillow.rois2masks([x.T for x in self.polys], self.data)
        # assert equality
        self.assert_equal(actual, self.expected)

    def test_masks(self):
        # load from masks
        actual = datahandlerpillow.rois2masks(self.expected, self.data)
    ...
    def test_rois_not_list(self):
        # check that rois2masks fails when the rois are not a list
        with self.assertRaises(TypeError):
            datahandlerpillow.rois2masks({}, self.data)
        with self.assertRaises(TypeError):
            datahandlerpillow.rois2masks(self.polys[0], self.data)
    ...
    def test_polys_not_2d(self):
        ...
        with self.assertRaises(ValueError):
            datahandlerpillow.rois2masks(polys1d, self.data)
        polys3d = [
            np.array([[39., 62., 0.], [60., 45., 0.], [48., 71., 0.]]),
            np.array([[72., 107., 0.], [78., 130., 0.], [100., 110., 0.]]),
        ]
        with self.assertRaises(ValueError):
            datahandlerpillow.rois2masks(polys3d, self.data)
