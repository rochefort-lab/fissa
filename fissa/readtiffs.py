...
def get_frame_number(img):
    ...
    '''
    # keep going up one frame, until no such frame exists (resulting in EOFEerror)
    while True:
        try:
            img.seek(img.tell()+1)
        except EOFError:
...
def getavg(img,box,frames):
   ...
   # where to store average
    ...
    for i,f in enumerate(frames):
        # get frame (note: best to now get all relevant data from this frame, so don't have to seek again (although takes neglible time)
        img.seek(f) 
        avg[:]+=img.crop(box)
    avg = avg/len(frames)
